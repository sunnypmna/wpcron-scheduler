...
PACKAGE_NAME="wpcron-scheduler-phase2"
OUTPUT="${1:-$HOME/${PACKAGE_NAME}.tar.gz}"
...
DEST="$TMP_DIR/$PACKAGE_NAME"
mkdir -p "$DEST"

cp -a "$SCRIPT_DIR/bin" "$DEST/"
cp -a "$SCRIPT_DIR/config" "$DEST/"
cp -a "$SCRIPT_DIR/lib" "$DEST/"
cp -a "$SCRIPT_DIR/VERSION" "$DEST/"
cp -a "$SCRIPT_DIR/README.md" "$DEST/"

rm -rf "$DEST"/lib/__pycache__
...
python3 -m py_compile "$DEST"/lib/*.py

rm -f "$OUTPUT"

tar -C "$TMP_DIR" -czf "$OUTPUT" "$PACKAGE_NAME"
